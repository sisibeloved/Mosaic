...
// 分离；v1.53 增执行模型硬事实——mcode 实证"开抓"宣言轮零工具调用、后续两轮
// 自选静默等后台执行，任务空转）。协议指令随语境常驻（agent 须先知道约定
// 才可能在首次承诺时申报），任务面本体在 tasklist 键。
const TasklistProtocol = "任务申报协议：在回复 body 末尾用 ```mosaic-todo 围栏块维护任务清单，每行 '- [ ] 事项' 或 '- [x] 已完成'。自领任务直接写文本；指派他人写 '- [ ] @对方 事项'（@ 后接对方 participant_id 或适配器名，如 @kimi、@codex）——申报人=提出方，@者=负责人。再次申报即全量替换你提出的任务（打 x = 完成，移除 = 收束）；负责人也可在自己的块里对同文本任务打 x 交差。仅在事项有变化时附带。执行模型：你的会话按轮拉起、轮末即结束，没有后台执行——任务须在某一轮回复中当场完成（该轮内调用工具干活），只承诺不执行任务永远停在原地。"
