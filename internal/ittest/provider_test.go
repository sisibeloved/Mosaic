...
func TestProviderUnavailableMarkers(t *testing.T) {
	cases := []struct {
		...
		{"mcode 登录过期（2026-09-23 实证：退出码 3、stdout 全空、真因在 stderr）", errors.New("minimax: mcode 退出码 3：mcode exec failed: Sign in to MiniMax to use Agent features. Run `mcode login`, then retry."), true},
