...
// v1.70：记忆评审映射 + generate 位 history_query 两段式出口（与 kimi/minimax 同口径）。
func TestMemoryReviewAndHistoryQuery(t *testing.T) {
	stream := `{"type":"thread.started","thread_id":"thr_x"}` + "\n" +
		`{"type":"item.completed","item":{"id":"i","type":"agent_message","text":"{\"ops\":[{\"action\":\"add\",\"content\":\"用户偏好简短回复\"}],\"public_rationale\":\"沉淀\"}"}}` + "\n" +
		`{"type":"turn.completed"}`
	exec := &fakeExecer{outputs: []string{stream}}
	adapter := newTestAdapter(exec)
	session, _ := adapter.Boot(context.Background(), agent.Profile{ProfileID: "p", Adapter: "codex"})
	defer session.Close()
	h, err := session.Run(context.Background(), agent.Task{TaskID: "t", Kind: agent.KindReviewMemory})
	if err != nil {
		t.Fatalf("run: %v", err)
	}
	res, rerr := h.Result()
	...
	qstream := `{"type":"thread.started","thread_id":"thr_y"}` + "\n" +
		`{"type":"item.completed","item":{"id":"i","type":"agent_message","text":"{\"history_query\":\"早期结论\"}"}}` + "\n" +
		`{"type":"turn.completed"}`
	exec2 := &fakeExecer{outputs: []string{qstream}}
	session2, _ := newTestAdapter(exec2).Boot(context.Background(), agent.Profile{ProfileID: "p2", Adapter: "codex"})
	defer session2.Close()
	h2, _ := session2.Run(context.Background(), agent.Task{TaskID: "t2", Kind: agent.KindGenerate})
	res2, err2 := h2.Result()
	if err2 != nil {
