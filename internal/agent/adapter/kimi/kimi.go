...
func mapResult(kind agent.TaskKind, parsed Parsed) (agent.Result, error) {
	...
	case agent.KindReplyOrPass:
		...
		if action != "speak" && action != "pass" {
			return agent.Result{}, fmt.Errorf("kimi: reply_or_pass 缺字段 action（speak|pass）")
		}
		if action == "speak" {
			body, _ := data["body"].(string)
			if strings.TrimSpace(body) == "" {
				return agent.Result{}, fmt.Errorf("kimi: reply_or_pass speak 缺正文")
			}
		}
		return agent.Result{Block: "reply_or_pass_decision", Data: data}, nil
	case agent.KindEvaluateIntent:
		data, err := agent.ExtractJSON(text)
		if err != nil {
			return agent.Result{}, err
		...
		agent.CapIntentRationale(data) // 超长 rationale 截断保决策（与 codex/minimax 同口径）
	...
	case agent.KindGenerate:
		...
		if data, err := agent.ExtractJSON(text); err == nil {
			...
			if strings.TrimSpace(body) == "" {
				...
				return agent.Result{}, fmt.Errorf("kimi: generate 输出为 JSON 但缺可用 body（不发布）")
			}
		...
		if agent.LooksLikeJSON(text) {
			return agent.Result{}, fmt.Errorf("kimi: generate 输出呈 JSON 形状但解析失败（不发布）")
		}
		// 纯文本回退：正文即发言
		return agent.Result{
			Block: agent.BlockPublicDraft,
	...
	case agent.KindReviewMemory:
		...
		return agent.Result{Block: agent.BlockMemoryOps, Data: data}, nil
	case agent.KindSummarize:
		if data, err := agent.ExtractJSON(text); err == nil {
