...
func mapResult(kind agent.TaskKind, parsed Parsed) (agent.Result, error) {
	...
	case agent.KindReplyOrPass:
		...
		if action != "speak" && action != "pass" {
			return agent.Result{}, fmt.Errorf("mcode: reply_or_pass 缺字段 action（speak|pass）")
		}
		if action == "speak" {
			body, _ := data["body"].(string)
			if strings.TrimSpace(body) == "" {
				return agent.Result{}, fmt.Errorf("mcode: reply_or_pass speak 缺正文")
			}
		}
	...
	case agent.KindGenerate:
		...
		if agent.LooksLikeJSON(text) {
			return agent.Result{}, fmt.Errorf("minimax: generate 输出呈 JSON 形状但解析失败（不发布）：%s", firstLineOf(text, 120))
		}
		// 纯文本回退：正文即发言
		return agent.Result{
			Block: agent.BlockPublicDraft,
