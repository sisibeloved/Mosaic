...
func TestSpecCommandKindEnum(t *testing.T) {
	...
	want := map[string]bool{"create_room": true, "post_message": true, "pause_room": true, "resume_room": true, "rename_room": true, "set_policy": true, "endorse_intent": true, "fork_thread": true, "pause_thread": true, "resume_thread": true, "close_thread": true, "reopen_thread": true, "merge_thread": true}
	if len(got) != len(want) {
