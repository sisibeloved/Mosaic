...
// 结构冷却拆除）、双边链无环闸继续（v1.43 对话环退役）、意愿静默终止、
// 逐发言人生成时语境刷新。
package room
...
// 对话环检测已整体退役（v1.43 负责人裁定：双边辩论是目标场景，无病理实录不设
// 闸）：双边互答链照常开波——终止只由意愿静默治理（人类暂停随时掐断）。
func TestChatDyadChainContinuesWithoutRingGate(t *testing.T) {
	store := NewMemStore()
	sup := agent.NewSupervisor()
	_ = sup.Register(silentAdapter{})
	defer sup.Shutdown()
	eng := NewEngine(EngineConfig{
		Store: store, Reader: store, Agents: sup,
		Seats: []AgentSeat{
			{ParticipantID: "par_a", Profile: agent.Profile{ProfileID: "pa", Adapter: "silent_stub"}},
	...
	seedRoomCreatedFor(t, store, "room_dyad")
	var chain []protocol.Envelope
	for i := 0; i < 6; i++ { // 旧计数闸（≥6）与形制短闸在此长度必命中——如今必须照常开波
		pid := "par_a"
		if i%2 == 1 {
			pid = "par_b"
		}
		chain = append(chain, protocol.Envelope{
			EventID: "evt_dyad_" + jsonNumber(int64(i)), TenantID: "ten_local", RoomID: "room_dyad",
			Type: protocol.EventMessagePosted, SchemaVersion: 1, OccurredAt: testClock(),
			Actor:      protocol.Actor{ParticipantID: pid, Kind: "agent"},
			Visibility: protocol.Visibility{Kind: "public"}, Payload: []byte(`{"body":"d"}`), Metadata: map[string]any{},
		})
	}
	if _, err := store.AppendEvents(context.Background(), chain); err != nil {
		t.Fatalf("append dyad: %v", err)
	}

	// 白盒直驱反应波：双边尾部照常开波（环闸退役），silent 座自决静默 → quiescent
	eng.runReaction(context.Background(), "room_dyad", false)
	time.Sleep(50 * time.Millisecond)
	events := store.RoomEvents("room_dyad")
	if n := countType(events, protocol.EventRoundOpened); n != 1 {
		t.Fatalf("双边链应照常开波（环闸已退役）：round.opened = %d（期望 1）：%v", n, typesOf(events))
	}
	for _, ev := range events {
		if ev.Type != protocol.EventRoundClosed {
			continue
		...
		if rc.Outcome != "quiescent" {
			t.Fatalf("silent 座应意愿静默收束：%s", rc.Outcome)
		}
	}
}
