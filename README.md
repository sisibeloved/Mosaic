...
当前版本 **v0.2.1**（2026-09-24，ZCode 接入（CLI 与桌面版），见 [CHANGELOG](CHANGELOG.md)）：核心讨论闭环、收束与记忆、产品化功能面（M0–M4）已完成并验收。后续版本规划：

- **0.3.0 Agent 兼容性扩充**：新增 Claude Code 等 Harness 适配器（ZCode 已于 0.2.1 提前交付，CLI 与桌面版双形态），扩大自动发现与登录探测的覆盖渠道。
