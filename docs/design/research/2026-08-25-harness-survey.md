...
| 已知编排陷阱 | `--json` 事件 schema 无稳定性承诺【社区】 | headless 缺失（#29）；Native Agent 不触发 config.json hooks（#32）【社区】 | 异常退出破坏 session 映射【社区】 | 子进程调用需关闭 stdin，否则可能挂起【社区】 | json 输出在个别非交互场景有 bug 报告【社区】 |
